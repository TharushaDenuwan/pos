...
export const list: AppRouteHandler<ListRoute> = async (c) => {
  ...
  const normalizedEntries = housingEntries.map((entry) => ({
    ...
    status: (entry as any).status ?? null, // Cast to any to access status if not in type
    createdAt:
...
const housingCreateSchema = z.object({
  ...
  status: z.string().nullable().default(null), // Always present, nullable
...
export const create: AppRouteHandler<CreateRoute> = async (c) => {
  const parsed = housingCreateSchema.safeParse(c.req.valid("json"));
  if (!parsed.success) {
    return c.json(
  ...
  const housingEntry: typeof housing.$inferInsert = parsed.data;
  ...
  const insertData = {
    ...
    // agentProfile: session.activeOrganizationId, // Removed because session does not have this property
    // Add any other required fields with default values if needed
  };
...
export const getOne: AppRouteHandler<GetOneRoute> = async (c) => {
  ...
  const housingEntry = await db.query.housing.findFirst({
    ...
    columns: {
      // Explicitly select 'status' along with other fields to ensure it's present
      id: true,
      title: true,
      address: true,
      price: true,
      ...
      updatedAt: true,
      agentProfile: true,
      status: true, // <-- Ensure status is selected
      // Add other fields as needed
    },
  });
